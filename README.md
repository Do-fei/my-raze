...
官方角色 Raze 的聊天页现在带 Live2D 舞台：视线跟随、点击互动、倾听/思考/说话四态，以及按心情和回复情绪切换的表情。点她或点底部按钮可以看生气、害羞、撒娇、哭鼻子、发脾气。API 语音会按音量张嘴；浏览器内置语音用模拟口型。设置里可以改回静态立绘。自建角色仍用照片（Live2D 授权不允许用户导入模型）。
